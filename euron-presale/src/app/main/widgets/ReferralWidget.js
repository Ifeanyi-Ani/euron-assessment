...
function ReferralWidget({ isManager }) {
  ...
  const [refUrl, setRefUrl] = useState(
    isManager ? defaultManagerUrl : defaultUrl,
  );
  ...
  const getReferralCodeIfExsits = (address) => {
    const apiURL = isManager
      ? process.env.REACT_APP_API_URL + "/manager/address/" + address
      : process.env.REACT_APP_API_URL + "/address/" + address;
    axios
      .get(apiURL)
      .then((response) => {
        if (response.status == 200) {
          ...
          setRefUrl(
            isManager
              ? defaultManagerUrl + response.data.ref_code
              : defaultUrl + response.data.ref_code,
          );
  ...
  const fetchChainData = async () => {
    ...
    if (isManager) {
      ...
      let managerReferralCount = await presaleContract.managerRefereeCount(
        address,
      );
      setManagerReferralCount(fromBigNum(managerReferralCount, 0));

      let managerBonusEarned = await presaleContract.managerBonusRequested(
        address,
      );
  ...
  const handleCreateRefCode = async () => {
    ...
    storeReferralDataToDB({
      address: address,
      ref_code: refCode,
      is_manager_code: isManager,
    });
  };

  const storeReferralDataToDB = async (formData) => {
    const apiURL = isManager
      ? process.env.REACT_APP_API_URL + "/manager"
      ...
      .then((response) => {
        if (response.status == 201) {
          ...
          dispatch(
            showMessage({
              message: isManager
                ? "Your manager code was created successfully!"
                : "Your referral code was created successfully!",
              variant: "success",
              anchorOrigin: {
                vertical: "top",
                horizontal: "right",
    ...
    <Paper className="flex flex-col flex-auto p-24 shadow rounded-2xl overflow-hidden h-full">
      ...
      <div className="flex flex-col flex-auto mt-10 mb-10">
        ...
        <FormHelperText id="component-helper-text">
          {isManager
            ? "Manager code should be start with a letter and be at least 5"
            : "Referral code should be start with a letter and be at least 5"}{" "}
          characters with a mix of numbers and letters, NO symbols
        </FormHelperText>
      </div>
      <div className="flex flex-col flex-auto mt-20 mb-20">
        {!isConnected ? (
          ...
          <div className="flex flex-col flex-auto mt-10 mb-10 items-center">
            <Typography>
              {isManager
                ? "Manager code creation valid only after purchase!"
      ...
      {isConnected && isBuyer ? (
        <>
          ...
          >
            ...
            <Link
              href={
                "https://x.com/Euron_Eth?t=iNFhaBEEdU7DCXV06s1yiA&s=09"
                // "https://twitter.com/intent/tweet?text=%F0%9F%8E%89+I+just+participated+in+%40vaultfi_io+presale%21+%F0%9F%8E%89%0D%0A%0D%0AUse+my+link+and+get+5%25+bonus+tokens%21+%0D%0Ahttps%3A%2F%2Fvaultfi-presale-web.vercel.app%2Fpresale%3Fr%3D$" +
                // refCode +
                // "%0D%0A%0D%0APre-sale+will+end+once+all+tokens+are+sold-out.+Don%27t+miss+out%21%0D%0A%0D%0A%40vaultfi_io+is+a+fully+audited+DeFi+protocol+with+APY+backed+by+revenue+generating+businesses+led+by+%40coinrockcap%0D%0A%0D%0A%23VaultFi+%23DeFi+%23CoinRock+%23Presale"
              }
            ...
            <Link
              href={
                "https://www.instagram.com/euronofficialeth/profilecard/?igsh=MThhbnpjOXdyc3pndA=="
                // "https://twitter.com/intent/tweet?text=%F0%9F%8E%89+I+just+participated+in+%40vaultfi_io+presale%21+%F0%9F%8E%89%0D%0A%0D%0AUse+my+link+and+get+5%25+bonus+tokens%21+%0D%0Ahttps%3A%2F%2Fvaultfi-presale-web.vercel.app%2Fpresale%3Fr%3D$" +
                // refCode +
                // "%0D%0A%0D%0APre-sale+will+end+once+all+tokens+are+sold-out.+Don%27t+miss+out%21%0D%0A%0D%0A%40vaultfi_io+is+a+fully+audited+DeFi+protocol+with+APY+backed+by+revenue+generating+businesses+led+by+%40coinrockcap%0D%0A%0D%0A%23VaultFi+%23DeFi+%23CoinRock+%23Presale"
              }
            ...
            <Link
              href={
                "https://t.me/EuronGroupDiscussion/"
            ...
            <Link
              href={
                "https://www.tiktok.com/@euronofficial?_t=8rOuOIqmOYS&_r=1"
                // "https://t.me/share/url?url=https%3A%2F%2Fvaultfi-presale-web.vercel.app%2Fpresale%3Fr%3D$" +
                // refCode +
                // "&text=%F0%9F%8E%89+I+just+participated+in+%40vaultfi_io+presale%21+%F0%9F%8E%89%0D%0A%0D%0AUse+my+link+and+get+5%25+bonus+tokens%21+%0D%0A"
              }
